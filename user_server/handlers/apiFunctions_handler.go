...
func AddUser_handler(w http.ResponseWriter, r *http.Request) {
	...
	CreateProfile(tokenString, createdUser)

}

func CreateProfile(token string, user *models.User) {
	...
	PROFILE_URL := "http://localhost:8083/profile/add"

	req, err := http.NewRequest("POST", PROFILE_URL, bytes.NewBuffer(jsonData))
	if err != nil {
